...
@pytest.mark.parametrize(
    "mtime, mtime_parsed, mtime_index, mtime_error",
    [
...
@pytest.mark.parametrize(
    "gain, gain_parsed, gain_error",
    [
        (-2.1, -2.1, False),  # OK
    ],
)
@pytest.mark.parametrize(
    "clip, clip_parsed, clip_error",
    [
        (0, None, True),  # != 0
        (-2, -2, True),  # Positive
        (3.3, 3.3, False),  # OK
...
def test_task_params(
    ...
    if any((tps_error, mtime_error, gain_error, clip_error, tti_error)):
        ...
        task = Task(
            source=read_demo_sgy, traces_per_gather=tps, maximum_time=mtime, traces_to_inverse=tti, gain=gain, clip=clip
        )
